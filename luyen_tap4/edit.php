...
if (!empty($book_err) || !empty($intro_err) || !empty($detail_err) || !empty($page_err) || !empty($price_err) || !empty($image_err)) {
    header("location:form_edit.php?book_err=$book_err & intro_err=$intro_err & detail_err=$detail_err & page_err=$page_err & price_err=$price_err & image_err=$image_err");
    die;
}
require_once('db.php');
$sql = "UPDATE books SET book_title='$book_title',image = '$saveimg',intro='$intro',detail='$detail',page='$page',price='$price',cate_id='$cate_id'";
