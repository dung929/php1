...
if (!empty($book_title_err) || !empty($intro_err) || !empty($detail_err) || !empty($page_err) || !empty($price_err) || !empty($image_err)) {
    header("location:form_add_books.php?book_title_err=$book_title_err & intro_err=$intro_err & detail_err=$detail_err & page_err=$page_err & price_err=$price_err & image_err=$image_err");
    die;
}
require_once('db.php');

$sql = "INSERT INTO books (book_title,image,intro,detail,page,price,cate_id) VALUES ('$book_title','$filename','$intro','$detail','$page','$price','$cate_id')";
