...
?>
<form action="edit_books.php" method="POST" enctype="multipart/form-data">
    <div>
        <label for="">Book_title</label>
        <input type="text" name="book_title" value="<?= $books['book_title']?>">
        <?php if (isset($_GET['book_title_err'])) : ?>
            <span style="color: red"><?= $_GET['book_title_err'] ?></span>
        <?php endif ?>
    </div>
    <div>
    ...
    <div>
        ...
        <input type="radio" value=1 name="cate_id" checked <?php if ($books['cate_id'] == 1) {
                                                        echo 'checked';
                                                    } ?>>1
